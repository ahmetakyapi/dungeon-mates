...
// ── Line timing per scene ──
const LINE_DELAYS: Partial<Record<Scene, { first: number; rest: number }>> = {
  king: { first: 1800, rest: 2800 },
  ritual: { first: 1500, rest: 3000 },
  catastrophe: { first: 800, rest: 2200 },
  corruption: { first: 1500, rest: 2500 },
  present: { first: 1000, rest: 2000 },
};

// ── Calculate scene timing dynamically from dialogue ──
function calcSceneDuration(scene: Scene): number {
  const lines = SCENE_DIALOGUE[scene];
  const timing = LINE_DELAYS[scene];
  if (!timing || lines.length === 0) {
    // Scenes without dialogue
    if (scene === 'darkness') return 5500;
    return 0; // manual (present)
  }
  // first line delay + rest delays + extra buffer to read last line
  return timing.first + (lines.length - 1) * timing.rest + 2500;
}

const AUTO_SKIP_MS = 60000;
...
// ── Draw corrupted Karanmir (demon form) ──
...
// ── Draw monster (corrupted citizen) ──
...
function DarknessCanvas() {
  const draw = useCallback((ctx: CanvasRenderingContext2D, frame: number) => {
    ...
    for (let i = 0; i < 80; i += 6) px(ctx, i, 42, 1, 8, '#374151');

    const intensity = 0.3 + Math.sin(frame * 0.08) * 0.15;
...
function KingCanvas() {
  const draw = useCallback((ctx: CanvasRenderingContext2D, frame: number) => {
    ...
    // Guards
    px(ctx, 15, 26, 2, 3, '#9ca3af');
    px(ctx, 14, 23, 4, 3, '#9ca3af');
    px(ctx, 15, 22, 2, 2, '#fcd5b4');
    px(ctx, 14, 21, 4, 2, '#6b7280');
    px(ctx, 14, 20, 1, 1, '#fbbf24');
    px(ctx, 18, 20, 1, 1, '#fbbf24');
    px(ctx, 13, 22, 1, 8, '#9ca3af');
    px(ctx, 13, 21, 1, 1, '#d1d5db');
    ...
    for (let i = 10; i < 90; i += 15) px(ctx, i, 2, 2, 1, '#fbbf24');
  }, []);
...
function CatastropheCanvas() {
  const draw = useCallback((ctx: CanvasRenderingContext2D, frame: number) => {
    ...
    drawFirstFire(ctx, 45, 30, frame, 1.0);
    for (let i = 0; i < 5; i++) {
    ...
    if (isHuman) {
      ...
      px(ctx, kx + 4 + shake, ky + 1, 8, 2, '#fbbf24');
    } else {
...
function PresentCanvas() {
  const draw = useCallback((ctx: CanvasRenderingContext2D, frame: number) => {
    ...
    for (let i = 0; i < 10; i++) px(ctx, 3 + i * 10, 36, 3, 1, '#15803d');
...
function drawHeroArcher(ctx: CanvasRenderingContext2D, x: number, y: number, frame: number) {
  ...
  px(ctx, x + 11, y + 4 + walkY, 1, 1, '#d1d5db');
...
const PARTICLE_COLORS: Record<Scene, string[]> = {
...
export function PrologueCinematic({ onComplete }: PrologueCinematicProps) {
  ...
  // Crossfade: fade overlay opacity 0→1 before scene change, then 1→0 after
  ...
  // Scene progression with crossfade
  useEffect(() => {
    if (scene === 'present') return;
    const duration = calcSceneDuration(scene);
  ...
  const particles = useMemo(
    () => generateParticles(25, PARTICLE_COLORS[scene]),
    [scene],
  );
  ...
  return (
    <motion.div
      ...
      onClick={handleSkip}
    >
      {/* Background — crossfades via opacity transition */}
      <motion.div
        key={`bg-${scene}`}
        className={`absolute inset-0 bg-gradient-to-b ${BG_CLASS[scene]}`}
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
      ...
      {/* Catastrophe screenshake + flashes */}
      {scene === 'catastrophe' && (
        <>
          <motion.div
            className="pointer-events-none absolute inset-0"
          ...
          />
          {[0.1, 0.5, 1.2, 2.8].map((d, i) => (
            <motion.div
              key={d}
              className="pointer-events-none absolute inset-0"
              style={{ backgroundColor: i === 1 ? 'rgba(245, 158, 11, 0.25)' : 'rgba(220, 38, 38, 0.2)' }}
              initial={{ opacity: 0 }}
      ...
      {/* Crossfade overlay — fades to black between scenes */}
      <motion.div
